...
const edit_profile = options => {
  P.coroutine(function *(){
    ...
    if(!username){
        Notify({ value: "Nombre de usuario no debe estar vacío" })
    } else if(!email){
        Notify({ value: "Email no debe estar vacío" })
    } else if(uCount == 1 && username != susername){
        Notify({ value: "Ese nombre de usuario ya existe" })
    } else if(eCount == 1 && email != semail){
        Notify({ value: "Ese email ya existe" })
    } else {
...
const createNote = options => {
  ...
  if(!title || !content){
    Notify({ value: "No están los campos rellenos" })
  } else {
...
const editNote = options => {
  ...
  if(title == "" || content == "" ){
    Notify({ value: "Campos están vacíos" })
...
const follow = options => {
  ...
  post('/api/follow', { user, username })
    .then(s => {
      ...
      Notify({ value: "Apuntado a Seguidos" })
      done()
